...
final class RadioTest extends TestCase
{
    ...
    public function testEnClosedByLabelWithFalse(): void
    {
        ...
        $this->assertSame(
            $expected,
            Radio::widget()->for($this->formModel, 'int')->enclosedByLabel(false)->value(1)->render(),
        );
    }
    ...
    public function testRender(): void
    {
        ...
        HTML;
        $this->assertSame($expected, Radio::widget()->for($this->formModel, 'int')->value(1)->render());
    }
    ...
    public function testValues(): void
    {
        ...
        HTML;
        $this->assertSame($expected, Radio::widget()->for($this->formModel, 'bool')->value(false)->render());

        // value bool true
        $expected = <<<'HTML'
        <label><input type="radio" id="typeform-bool" name="TypeForm[bool]" value="1" checked> Bool</label>
        HTML;
        $this->assertSame($expected, Radio::widget()->for($this->formModel, 'bool')->value(true)->render());
        ...
        HTML;
        $this->assertSame($expected, Radio::widget()->for($this->formModel, 'int')->value(0)->render());
        ...
        HTML;
        $this->assertSame($expected, Radio::widget()->for($this->formModel, 'string')->value('inactive')->render());

        // value string 'active'
        $expected = <<<'HTML'
        <label><input type="radio" id="typeform-string" name="TypeForm[string]" value="active" checked> String</label>
        HTML;
        $this->assertSame($expected, Radio::widget()->for($this->formModel, 'string')->value('active')->render());

        // value null
        $expected = <<<'HTML'
        <label><input type="radio" id="typeform-tonull" name="TypeForm[toNull]" checked> To Null</label>
        HTML;
        $this->assertSame($expected, Radio::widget()->for($this->formModel, 'toNull')->value(null)->render());
    }
