...
### Example CheckboxList with label values `['0' => 'Female', [1] => 'Male']` and `'forceUncheckedValue' => 'undefined'`, returns an array, which can one value, or both values. 
...
```html
<form action="widgets" method="POST" _csrf="W7gBL2c6wIwImBjXCLDGUnVNzsUVnNR5d2ovC1sCkmYY0E1uNnSE_WLXR-RC77ZrB32Hq2OohE0mDHc5M23LPg==">
    <input type="hidden" name="_csrf" value="W7gBL2c6wIwImBjXCLDGUnVNzsUVnNR5d2ovC1sCkmYY0E1uNnSE_WLXR-RC77ZrB32Hq2OohE0mDHc5M23LPg==">
    ...
    <hr class="mt-3">
    <div>
        <input type="submit" id="submit-224894203881001" class="button is-block is-info is-fullwidth" name="submit-224894203881001" value="Save">
    </div>
</form>
```

As we can see in the code we have the value `undefined` for the input hidden for empty values submit, for the first checkbox and `Female` for the second checkbox `Male`, the possible return values are:
`
